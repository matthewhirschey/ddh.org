...
onedegree_nodes_id <-  as_tibble(graph_network) %>% 
  rowid_to_column("id") %>% 
  mutate(id = id -1, #req'd for d3
         group = 1) %>% 
  filter(name %in% graph_degree_1) %>% 
  pull(id)
...
nodes <-  as_tibble(graph_network) %>% 
  ...
  mutate(id = id -1, #req'd for d3
         group = 1) %>% 
  filter(!name %in% onedegree_nodes) %>% 
  as.data.frame

links <- graph_network %>% 
  activate(edges) %>% # %E>%
  as_tibble() %>% 
  mutate(from = from - 1, to = to - 1) %>% #req'd for d3
  filter(!from %in% onedegree_nodes_id | !to %in% onedegree_nodes_id ) %>% 
  as.data.frame

forceNetwork(Links = links, Nodes = nodes, Source = "from", Target ="to", NodeID = "name", Group = "group", zoom = TRUE, bounded = TRUE, opacityNoHover = 100)
```
