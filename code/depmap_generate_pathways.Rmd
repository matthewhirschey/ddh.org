...
set.seed(2019)
sample <- sample(names(achilles), size = 100)
```

#put data into lists
```{r reports_complete, eval=FALSE, warning=FALSE, include=FALSE}
gene_group <- c(purine, pyrimidine)
...
#master_table_top
```{r}
sample <- sample(names(achilles_cor), size = 100)
gene_group <- sample
...
#master_positive
FIX MASTER LISTS
They are pulling gene lists from dep_top, but there's only one there. 
So in this chunk, I repeated the same gene list look-up 100x
*need* to generate list (either pull from master top, or generate on the fly), and then pipe into it. Probably generate on the fly, so it can be parallel, and not rely on one of the other chunks.
```{r}
gene_group <- sample #defined in first block (line 318)
