...
#small sample
```{r}
sample <- sample(names(achilles_cor), size = 20)
...
for (fav_gene in gene_group) {
  ...
  target_achilles_bottom <- target_achilles %>% 
    ...
    #plot1:cell_bins
    cell_bins_plot <- ggplot(target_achilles) +
    ...
    #plot2:cell_deps
    cell_deps_plot <- ggplot(target_achilles) +
    ...
    cell_bins <- list(cell_bins_plot)
    cell_deps <- list(cell_deps_plot)
    plots <- tibble(fav_gene, cell_bins, cell_deps)
