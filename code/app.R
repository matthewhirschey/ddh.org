  ...
  navbarPage(title = main_title, windowTitle = "Data-Driven Hypothesis | A Hirschey Lab Resource", 
    ...
    navbarMenu(title = "Similar",
               tabPanel("Genes",
                        ...
                        sidebarLayout(
                          sidebarPanel(fluidRow(checkboxGroupInput(inputId = "vars_dep_top", 
                                                                   "Select table columns:",
                                                                   c("R^2", "Z Score", "Co-publication Count", "Co-publication Index"), 
                                                                   selected = c("Z Score", "Co-publication Count"), 
                                                                   inline = TRUE)),
                                       hr(),
                                       fluidRow(sliderInput(inputId = "num_sim_genes",
                                                            "Censor genes with more than:",
                                                            min = 100,
                                                            max = 1000,
                                                            value = 1000, 
                                                            step = 100)),
                                       fluidRow(column(5, actionButton(inputId = "censor",
                                                ...
                                                column(5, actionButton(inputId = "reset",
                                                                       "Reset"))),
                                       width = 3),
                          mainPanel(fluidRow(dataTableOutput(outputId = "dep_top"))), 
                          position = "right")),
               tabPanel("Pathways",
