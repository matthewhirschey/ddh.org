...
#dep tables
dep_top <- make_top_table(fav_gene)
dep_bottom <- make_bottom_table(fav_gene)
...
dep_plot1 <- make_celldeps(fav_gene)
dep_plot2 <- make_cellbins(fav_gene)
dep_plot1 + dep_plot2
```
...
The data presented on datadrivenhypothesis.org is an interactive image that can be zoomed, dragged, and manipulated for data exploration.
