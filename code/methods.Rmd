...
log_break_labels <-c("0", "1", "10", "100", "1000", "10,000", "100,000", "1,000,000")
joined %>% 
  #sample_n(10000) %>% 
  ggplot() +
...
As an example, we will query the protein P53. Typing in the given protein name "P53" produces an error, because the official gene symbol needs to be enetered. Querying TP53 (official gene symbol) generates a short summary of the gene, its name and list of aliases, and an entrez gene summary paragraph when available.  
...
The second plot is a histogram of dependency scores, showing the distribution of scores for `r fav_gene`. While the majority of cells have little change in celluar growth when `r fav_gene` is absent (the histogram is centered around zero), some cells require `r fav_gene` for growth (those below -1), whereas in other cells `r fav_gene` functions as a tumor suppressor (those above 1).  
...
```{r target_achilles_bottom}
target_achilles_bottom %>% 
  rename("Cell Line" = "cell_line", "Lineage" = "lineage", "Dependency Score" = "dep_score")
```
  
Understanding the shape of the curve and distribution of the raw data underlying the patterns is important.  
...
The interpretation of these genes and pathways is less consistent. In some cases, a negative regulator of a gene is shown, such as in this example with `r fav_gene` and its negative regulator `r dep_bottom$gene[1]`. In other cases, opposing pathways are shown, contrasting the `r fav_gene` enriched pathway term "`r flat_top_complete$Term[8]`" with the dissimilar enriched pathway term "`r flat_bottom_complete$Term[12]`", revealing two opposing biological pathways.  
