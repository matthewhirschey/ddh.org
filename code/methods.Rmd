...
#make raw correlation matrix
achilles_cor_raw <- achilles_raw %>% 
  select(-X1) %>% 
  correlate() 

achilles_cor_raw_long <- achilles_cor_raw %>% 
...
joined %>% 
  ...
  scale_fill_viridis_c(option = "C", name = "Count", trans = "log10", breaks = log_breaks, labels = log_labels) + #log scale transform legend
...
no0_plot <- ggplot(achilles_no0_plot) +
...
na_cutoff_tmp <- count_v_max %>% 
  top_frac(-.05, pos) %>% 
  arrange(pos) %>% 
  pull(NAs) %>% 
  min(.)

threshold_plot <- count_v_max_no0 %>% 
  ggplot() +
...
#did this in the previous chunk
...
achilles_joined %>% 
  ...
  geom_hex(aes(x = achilles_cor_raw, y = achilles_cor), 
           bins = 90, na.rm = TRUE, size = 0.2, color = "black") +
  scale_fill_viridis_c(option = "C", name = "Count", trans = "log10", breaks = log_breaks, labels = log_labels) + #breaks and labels defined above
