...
#find_threshold.R, 
#generate_depmap_data.R 
#generate_pubmed_data.R
#generate_depmap_stats.R
#then knit
...
heatmap_dep <- ggplot(data, aes(X, fct_rev(Y), fill= Z)) + 
  geom_tile() +
  labs(x = "", y = "", fill = "Dependency \nScore", subtitle = "Dependency Score Example*", caption = "*simulated data") +
  theme_minimal_grid(color = "white") + 
  scale_fill_viridis() + 
  scale_x_discrete(position = "top")
...
depmap_raw
ggsave(here::here("results", "fig2e_depmap_raw.png"), plot = depmap_raw, dpi = 300)
```
...
flat_top_complete <- make_enrichment_table(master_positive, fav_gene)
flat_bottom_complete <- make_enrichment_table(master_negative, fav_gene)
```
...
p53graph <- make_graph_report(master_top_table, master_bottom_table, fav_gene)
