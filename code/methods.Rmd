...
gene_location <- readRDS(here::here("data", paste0(release, "_gene_location.Rds")))
chromosome <- readRDS(here::here("data", paste0(release, "_chromosome.Rds")))
pubmed <- readRDS(file = here::here("data", paste0(release, "_pubmed.Rds")))
cellosaurus <- readRDS(file = here::here("data", paste0(release, "_cellosaurus.Rds")))
cellosaurus_key <- readRDS(file = here::here("data", paste0(release, "_cellosaurus_key.Rds")))

#read data from generate_depmap_data.R
achilles_long <- readRDS(file=here::here("data", paste0(release, "_achilles_long.Rds")))
achilles_cor_nest <- readRDS(file=here::here("data", paste0(release, "_achilles_cor_nest.Rds"))) #replaces achilles_cor
...
expression_long <- readRDS(file=here::here("data", paste0(release, "_expression_long.Rds"))) #replaces expression

#read drug names for search
prism_names <- readRDS(here::here("data", paste0(release, "_prism_names.Rds"))) 
prism_meta <- readRDS(here::here("data", paste0(release, "_prism_meta.Rds"))) 
...
depmap_raw <- 
  ...
  labs(x = "Dependency Score", 
       y = "Count (Million)", 
       subtitle = paste0("Dependency Score Distribution from DepMap release ", release)) +
...
sampling
#ggsave(here::here("results", "fig2d_sampling.png"), plot = sampling, dpi = 300)
```
...
#dep tables
dep_top <- make_top_table(input = list(type = "gene", query = fav_gene))
dep_bottom <- make_bottom_table(input = list(type = "gene", query = fav_gene))
...
The second plot is a density plot of dependency scores, showing the distribution of scores for `r fav_gene`. While the majority of cells have little change in cellular growth when `r fav_gene` is absent (the density plot is centered around zero), some cells require `r fav_gene` for growth (cells scoring below -1), whereas in other cells `r fav_gene` functions as a tumor suppressor (cells with a score above 1).  
  
  
```{r dep_plots, echo=FALSE, message=FALSE, warning=FALSE}
dep_plot1 <- 
  make_celldeps(input = list(type = "gene", 
                             query = fav_gene, 
                             mean = mean_virtual_achilles))

dep_plot2 <- 
  make_cellbins(input = list(type = "gene", 
                             query = fav_gene))
dep_plot1
dep_plot2
...
#### Cells with that show no fitness loss (or sometimes gains) upon `r fav_gene` ablation
```{r target_achilles_bottom, echo=FALSE, message=FALSE, warning=FALSE}
achilles_long %>%  
  dplyr::filter(gene == fav_gene) %>% 
  dplyr::left_join(expression_names, by = "X1") %>% 
  dplyr::select(cell_line, lineage, dep_score) %>% 
  dplyr::arrange(desc(dep_score)) %>% 
  dplyr::top_n(dep_score, n = 10) %>% 
  dplyr::mutate(dep_score = round(dep_score, 2)) %>% 
  dplyr::rename("Cell Line" = "cell_line", "Lineage"= "lineage", "Dependency Score" = "dep_score") %>% 
  gt() %>% 
...
head(dep_top, 10) %>% 
  dplyr::select("Gene" = "gene", "Name" = "name", "R^2" = "r2", "Concept Count" = "concept_count") %>% 
  gt() %>% 
  opt_row_striping
```
...
How to interpret these genes and pathways is more variable than the positively correlated genes and pathways. In some cases, a negative regulator of a gene has a negative correlation with that gene, such as in this example with `r fav_gene` and its negative regulator `r dep_bottom$Gene[1]`. In other cases, opposing *pathways* are shown, contrasting the `r fav_gene` enriched pathway term "`r flat_top_complete[11,3]`" with the dissimilar enriched pathway term "`r flat_bottom_complete[11,3]`", revealing two opposing biological pathways.    
...
make_graph(input = list(type = "gene", query = "TP53"), corrType = "Positive") %>% 
...
The Hirschey lab wrote a [pre-print](https://www.biorxiv.org/content/10.1101/2020.07.17.208751v1) describing this early resource. Furthermore, the Broad Institute has information on their website dedicated to the [Dependency Map project](http://www.depmap.org) about how the raw data were generated, and provide a list of references.  
