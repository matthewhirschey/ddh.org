...
mitocarta_full <- read_feather(here("raw", "mitocarta.feather"))
mitocarta <- mitocarta_full %>% pull(Symbol)

transporter <- read_xlsx(here("raw", "Transporter_genes_list.xlsx"), col_names = TRUE) %>% 
  clean_names() %>% 
  pull(gene)

#recursive; run after defining achilles_cor to create a vector of top/bottom genes for reports
#top20 <- achilles_cor %>% 
...
ggsave("output/expression_v_dep.png", plot = last_plot(), dpi = 600)

ggplot(joined) +
  geom_hex(aes(x = dep_score, y = gene_expression, fill = ..value..), 
           bins = 100, na.rm = TRUE, size = 0.2, color = "black") +
  scale_fill_viridis_c(option = "C") +
  labs(x = "Dependency Score", y = "Gene Expression") +  
  coord_fixed() +
  theme_minimal()

ggsave("output/expression_v_dep2.png", plot = last_plot(), dpi = 600)
