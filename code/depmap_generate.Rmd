...
expression <- read_csv(params$ccle_url, col_names = TRUE) %>% 
  `colnames<-`(str_remove_all(names(.), "\\s\\(\\d+\\)"))
write_feather(expression, path = here("data", paste0(params$release, "_expression.feather")))

expression_id <- read_csv(params$cclemeta_url, col_names = TRUE) %>% 
  clean_names
write_feather(expression_id, path = here("data", paste0(params$release, "_expression_id.feather")))
...
write_feather(achilles_cor, path = here("data", paste0(params$release, "_achilles_cor.feather")))


#generate graph dataset here
achilles_cor_long <- achilles_cor %>% 
  stretch() #310M observations across 3 variables (x, y, r)
write_feather(achilles_cor_long, path = here("data", paste0(params$release, "_achilles_cor_long.feather")))
...
write_feather(expression_cor, path = here("data", paste0(params$release, "_expression_cor.feather")))
...
expression_cor_long <- expression_cor %>% 
  stretch() #310M observations across 3 variables (x, y, r)
write_feather(expression_cor_long, path = here("data", paste0(params$release, "_expression_cor_long.feather")))
